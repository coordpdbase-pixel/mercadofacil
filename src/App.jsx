...
const css = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .pill-orange{background:#FFF3E0;color:${C.tangerine};}
  ...
  .photo-area{border:2px dashed ${C.border};border-radius:12px;padding:20px;text-align:center;cursor:pointer;background:${C.cream};margin-bottom:12px;transition:border .2s;}
  .photo-area:hover{border-color:${C.green};}
...
async function sbGet(table,extra=""){
  const r=await fetch(`${SUPABASE_URL}/rest/v1/${table}?select=*${extra}`,{headers:H});
  return r.json();
}
...
async function uploadFoto(file){
  ...
  const path=`${uid()}.${ext}`;
  const r=await fetch(`${SUPABASE_URL}/storage/v1/object/fotos/${path}`,{
    method:"POST",
...
export default function App(){
  ...
  function handleLogin(pass){
    if(pass===ADMIN_PASS){setIsAdmin(true);setShowLogin(false);showToast("👑 Bem-vinda, admin!");} 
    ...
    <>
      ...
      {promos.map(p=>(
        <div className="promo-card" key={p.id}>
          {p.foto_url
            ?<img src={p.foto_url} alt={p.nome} className="promo-img"/>
  ...
  return(
    <div className="overlay" onClick={onClose}>
      <div className="sheet" onClick={e=>e.stopPropagation()}>
        ...
        {isAdmin&&<><div style={{marginBottom:10}}><div className="label">Nome</div><input className="input" placeholder="Ex: Frango em promoção" value={nome} onChange={e=>setNome(e.target.value)}/></div>
        <div style={{marginBottom:10}}><div className="label">Descrição</div><input className="input" placeholder="Detalhes da promoção..." value={descricao} onChange={e=>setDescricao(e.target.value)}/></div>
        ...
        {pendentes.map(p=>(
          <div className="pending-card" key={p.id}>
            ...
            <div style={{fontSize:12,color:C.muted,marginBottom:10}}>
              {p.mercado&&<span>🏪 {p.mercado} </span>}
              {p.preco&&<span style={{color:C.tangerine,fontWeight:700}}>{fmtBRL(p.preco)}</span>}
              <span style={{marginLeft:8}}>{fmtDate(p.ts)}</span>
            </div>
            <div style={{display:"flex",gap:8}}>
...
function HistoryTab({prices,markets}){
  if(prices.length===0) return(<div className="empty"><div className="empty-icon">📈</div><div className="empty-title">Sem histórico ainda</div><p>Registre preços e eles aparecerão aqui.</p></div>);
  const groups={};
  prices.forEach(p=>{const key=p.name.toLowerCase().trim();if(!groups[key])groups[key]={name:p.name,entries:[]};groups[key].entries.push(p);});
  return(
    <>
          ...
          <div className="card" key={g.name}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
              ...
              <div style={{fontSize:13,fontWeight:700}}>
                <span style={{color:C.green}}>{fmtBRL(minP)}</span>{" / "}<span style={{color:C.red}}>{fmtBRL(maxP)}</span>
              </div>
            </div>
...
function ListTab({items,onToggle,onDelete,prices,markets}){
  ...
  function bestFor(name){const m=prices.filter(p=>p.name.toLowerCase().includes(name.toLowerCase().slice(0,5)));return m.length?m.reduce((a,b)=>a.price<b.price?a:b):null;}
  const Row=({item})=>{
  ...
  if(items.length===0) return(<div className="empty"><div className="empty-icon">🛒</div><div className="empty-title">Lista vazia</div><p>Toque em "+" para adicionar itens.</p></div>);
  return(
    <>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
      ...
      <div className="card">
        ...
        {done.length>0&&(<><div className="divider"/><div style={{fontSize:12,color:C.muted,marginBottom:8,fontWeight:600}}>Comprados</div>{done.map(item=><Row key={item.id} item={item}/>)}</>)}
      </div>
    </>
  );
}
...
function MarketsTab({markets,prices,onAdd,onRemove,showToast}){
  ...
  return(
    <>
      ...
      {markets.length===0
        ?<div className="empty"><div className="empty-icon">🏪</div><div className="empty-title">Nenhum mercado</div></div>
...
function AddItemSheet({onClose,onSave}){
  ...
  const [qty,setQty]=useState("1");
  const [saving,setSaving]=useState(false);
  async function handle(){if(!name.trim())return;setSaving(true);await onSave({id:uid(),name:name.trim(),qty,done:false,ts:Date.now()});setSaving(false);}
  return(
    <div className="overlay" onClick={onClose}>
      <div className="sheet" onClick={e=>e.stopPropagation()}>
        <div className="sheet-handle"/>
        <div className="sheet-title">Adicionar à lista</div>
        <div style={{marginBottom:12}}><div className="label">Produto</div><input className="input" placeholder="Ex: Leite, Ovos, Arroz..." value={name} onChange={e=>setName(e.target.value)} autoFocus/></div>
        <div style={{marginBottom:16}}><div className="label">Quantidade</div><input className="input" placeholder="1" inputMode="numeric" value={qty} onChange={e=>setQty(e.target.value)}/></div>
